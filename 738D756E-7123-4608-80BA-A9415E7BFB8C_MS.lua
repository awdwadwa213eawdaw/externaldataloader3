...
--					table.insert(self.ObtainedItems, item)
	...
	if self.Hits <= 0 then
		...
		Utilities.fastSpawn(function()
			msg1, msg2 = _p.Connection:get('PDS', 'finishDig', self.hBuf:ToBase64())
			returned = true
		end)
		if not dugAll then
			local rumble = true
		...
		Utilities.fastSpawn(function() nextDig = _p.Connection:get('PDS', 'nextDig') end)
...
function MiningSystem:ToggleSubmarine(on)
	self.SubmarineOn = on
	local mw, bubbles = _p.Connection:get('ToggleSubmarine', on)
	if on then
					...
					local button = _p.RoundedFrame:new {
						...
						MouseButton1Click = function()
							if products[i] == 1 then
								local buyUMV1 = _p.Connection:get('PDS', 'Purchase', "UMV1")
								if not buyUMV1 then return end
							elseif products[i] == 2 then
								local buyUMV3 = _p.Connection:get('PDS', 'Purchase', "UMV3")
								if not buyUMV3 then return end
							elseif products[i] == 3 then
								local buyUMV6 = _p.Connection:get('PDS', 'Purchase', "UMV6")
								if not buyUMV6 then return end
							end
						end,
						create 'ImageLabel' {
							BackgroundTransparency = 1.0,
